...
from tensorflow.keras.layers import Input, Activation, MaxPooling2D, UpSampling2D, Lambda
...
#
#     return X
def up_and_concatenate(down_layer, layer):
    # up = Conv2DTranspose(out_channel, [2, 2], strides=[2, 2])(down_layer)
    up = UpSampling2D(size=(2, 2))(down_layer)

    my_concat = Lambda(lambda x: K.concatenate([x[0], x[1]], axis=3))

    concatenate = my_concat([up, layer])
...
#         features = features // 2
#         x = up_and_concatenate(x, skips[i])
#         x = rec_res_block(x, features)
#
